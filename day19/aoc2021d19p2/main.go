...
func (p *point3D) ManhattenDist(other point3D) int {
	dx := other.X - p.X
	dy := other.Y - p.Y
	dz := other.Z - p.Z
	if dx < 0 {
...
func solvePuzzle(r io.Reader) (int, error) {
	...
	maxDist := 0
	for i, t1 := range scannerTransforms {
		for j, t2 := range scannerTransforms {
			if i == j {
				continue
			}
			dist := t1.globalPos.ManhattenDist(t2.globalPos)
			if dist > maxDist {
				maxDist = dist
			}
		}
